    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('study-sessions.update', $studySession) }}" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <select id="status" 
                                ...
                                <option value="planned" 
                                        {{ old('status', $studySession->status) == 'planned' ? 'selected' : '' }}>
                                    Planejada
                                </option>
                                <option value="in_progress" 
                                        {{ old('status', $studySession->status) == 'in_progress' ? 'selected' : '' }}>
                                    Em Andamento
                                </option>
                                <option value="completed" 
                                        {{ old('status', $studySession->status) == 'completed' ? 'selected' : '' }}>
                                    Concluída
                                </option>
                                <option value="cancelled" 
                                        {{ old('status', $studySession->status) == 'cancelled' ? 'selected' : '' }}>
                                    Cancelada
                                </option>
                            </select>
